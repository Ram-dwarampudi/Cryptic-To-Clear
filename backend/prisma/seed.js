...
async function main() {
  console.log("🌱 Starting database seeding (safe upsert mode — real users are preserved)...");

  // NOTE: We use upsert instead of deleteMany so real registered users are NEVER lost.
  ...
  const cseDept = await prisma.department.upsert({
    where: { universityId_code: { code: "CSE", universityId: uni.id } },
    update: {},
    create: {
  ...
  const csbsDept = await prisma.department.upsert({
    where: { universityId_code: { code: "CSBS", universityId: uni.id } },
    update: {},
    create: {
  ...
  // 3. Upsert Users (only creates if they don't exist — never deletes real registered users)
  const passwordHash = await bcrypt.hash("Password123!", 10);
  const facultyPasswordHash = await bcrypt.hash("Faculty123!", 10);

  const ramStudent = await prisma.user.upsert({
    where: { id: "77127a83-4add-435a-90f7-bf73471c23d2" },
    update: {},
    create: {
  ...
  const samrudhStudent = await prisma.user.upsert({
    where: { id: "99a7bcf2-7c11-4924-b9e3-5e959ffa5ce3" },
    update: {},
    create: {
  ...
  const saiStudent = await prisma.user.upsert({
    where: { id: "30743284-a08d-4236-b592-6d36213fadf5" },
    update: {},
    create: {
  ...
  const praneethaStudent = await prisma.user.upsert({
    where: { id: "b8fc6314-a8a3-4544-b73e-811c3e90f289" },
    update: {},
    create: {
  ...
  // 4. Seed Interview Experiences (skip if already seeded for these students)
  const existingInterviews = await prisma.interviewExperience.count({
    where: { studentId: { in: [ramStudent.id, samrudhStudent.id, saiStudent.id, praneethaStudent.id] } },
  });
  ...
  if (existingInterviews === 0) {
    ...
    console.log(`   ⏭️  Skipped interviews — ${existingInterviews} already exist.`);
  }

  // 5. Seed Doubt Posts and Answers (skip if already seeded)
  const existingDoubts = await prisma.doubt.count({
    where: { authorId: { in: [ramStudent.id, samrudhStudent.id, praneethaStudent.id] } },
  });
...
map.put(null, "value"); // Throws NullPointerException! Why?`,
    ...
    console.log("   ✅ Doubt threads seeded.");
  } else {
    console.log(`   ⏭️  Skipped doubts — ${existingDoubts} already exist.`);
  }

  console.log("✅ Seeding completed successfully!");
  console.log(`   - 1 University (${uni.name})`);
  console.log(`   - 2 Departments`);
  console.log(`   - 5 Core Users (1 Faculty, 4 Students) — existing real users preserved`);
  console.log(`   - Interview Experiences & Doubt Threads (skipped if already present)`);
}
