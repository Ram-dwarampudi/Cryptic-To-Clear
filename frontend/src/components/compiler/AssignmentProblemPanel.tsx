...
export default function AssignmentProblemPanel({
  ...
  if (!isOpen) {
    return (
      <div className="shrink-0 bg-[#0c1220] border-r border-zinc-800/80 flex flex-col items-center py-4 px-1.5 select-none">
        <button
          onClick={onToggle}
          title="Open Problem Statement & Constraints"
          className="p-2 rounded-xl bg-purple-500/15 text-purple-300 border border-purple-500/30 hover:bg-purple-500/25 transition-colors cursor-pointer flex flex-col items-center gap-1.5"
        >
          <BookOpen className="w-4 h-4" />
          <span className="text-[10px] font-mono [writing-mode:vertical-rl] rotate-180 tracking-wider font-bold">
            PROBLEM SPECS
          </span>
  ...
  return (
    <aside className="w-80 md:w-96 shrink-0 bg-[#090e1a] border-r border-zinc-800/90 flex flex-col h-full font-mono text-xs select-none overflow-hidden animate-in slide-in-from-left duration-200">
      {/* Panel Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800/80 bg-[#0d1424] shrink-0">
        <div className="flex items-center gap-2">
          <BookOpen className="w-4 h-4 text-purple-400" />
          <span className="font-bold text-zinc-100 text-[13px] tracking-tight">Assignment Specs</span>
        </div>
        <button
          onClick={onToggle}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 select-text custom-scrollbar">
        ...
        <div className="space-y-2">
          <h2 className="text-base font-display font-bold text-zinc-100 leading-snug">
            {assignment.title}
          </h2>
          ...
          <div className="flex flex-wrap items-center gap-1.5 pt-0.5">
            ...
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-amber-500/10 text-amber-300 border border-amber-500/25">
        ...
        <div className="p-3 rounded-xl bg-zinc-900/80 border border-zinc-800 text-[11.5px] space-y-1.5">
          <div className="flex items-center justify-between text-zinc-400">
            <span className="flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-zinc-500" />
              <span>Due Date:</span>
            </span>
            <strong className="text-zinc-200">{deadlineStr}</strong>
          </div>
          <div className="flex items-center justify-between text-zinc-400">
            <span className="flex items-center gap-1.5">
            ...
            <strong className="text-zinc-200">{assignment.maxAttempts || 5} attempts</strong>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="p-3.5 rounded-xl bg-[#0d1424] border border-zinc-800 text-[12.5px] leading-relaxed text-zinc-300 font-sans whitespace-pre-wrap">
        ...
        <div className="space-y-1.5">
          ...
          <div className="p-2.5 rounded-xl bg-zinc-900/60 border border-zinc-800 text-[11.5px] text-zinc-300">
            {isRestricted ? (
              <div className="flex items-center gap-1.5 flex-wrap">
                {assignment.allowedLanguages?.map((lang) => (
                  <span
                    key={lang}
                    className="px-2 py-0.5 rounded bg-purple-500/20 text-purple-300 border border-purple-500/30 text-[10.5px] font-bold uppercase"
                  >
              ...
              <span className="text-emerald-400 font-medium">Any Supported Language (Python, Java, C, C++, etc.)</span>
            )}
          </div>
        </div>
        ...
        <div className="space-y-2 pt-1">
          <div className="flex items-center justify-between">
            ...
            {hiddenCasesCount > 0 && (
              <span className="text-[10px] text-purple-400/90 font-medium bg-purple-500/10 px-2 py-0.5 rounded border border-purple-500/20">
          ...
          {revealedCases.length === 0 ? (
            <div className="p-3 rounded-xl bg-zinc-900/60 border border-zinc-800 text-[11.5px] text-zinc-500 italic">
            ...
            revealedCases.map((tc, idx) => (
              <div
                key={tc.id || idx}
                className="p-3 rounded-xl bg-[#0c1220] border border-zinc-800/80 space-y-2"
              >
                <div className="flex items-center justify-between text-[11px]">
                  <span className="font-bold text-zinc-300">Test Case #{idx + 1}</span>
                  {tc.explanation && (
                    <span className="text-[10px] text-zinc-500 italic">{tc.explanation}</span>
                  )}
                </div>
                ...
                <div className="space-y-1.5 text-[11px]">
                  <div>
                    <div className="flex items-center justify-between text-zinc-500 text-[10px] mb-0.5">
                      ...
                      <button
                        ...
                        className="hover:text-zinc-300 cursor-pointer flex items-center gap-1 text-[10px]"
                      >
                        {copiedIdx === idx * 2 ? (
                    ...
                    <pre className="p-2 rounded-lg bg-[#050810] border border-zinc-850 text-emerald-300 font-mono text-[11px] overflow-x-auto whitespace-pre-wrap">
                  ...
                  <div>
                    ...
                    <pre className="p-2 rounded-lg bg-[#050810] border border-zinc-850 text-cyan-300 font-mono text-[11px] overflow-x-auto whitespace-pre-wrap">
